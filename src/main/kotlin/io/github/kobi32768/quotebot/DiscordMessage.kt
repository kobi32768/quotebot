...
suspend fun MessageReceivedEvent.sendMessage(message: String) {
    this.channel.sendMessage(message).submit().await()
}

suspend fun MessageReceivedEvent.sendErrorMessage(error: Error) {
...
suspend fun MessageData.callForceQuote() {
    val guild = this.guild
    val user = this.event.author

    forceQuote(this, guild.findMembers { it.user == user }.await())
}
...
suspend fun sendRegularEmbedMessage(data: MessageData) {
    val message = data.message
    val event = data.event
    val member = runCatching { data.guild.retrieveMember(data.message.author).submit().await() }.getOrNull()
    ...
    event.channel.sendMessageEmbeds(embed.build()).submit().await()
}
